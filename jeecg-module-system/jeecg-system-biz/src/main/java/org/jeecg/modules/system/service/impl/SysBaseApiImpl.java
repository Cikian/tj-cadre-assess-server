...
//		}
    ...
    public List<SysDepartModel> getAllSysDepart() {
        ...
        // TODO: 框架源码被修改
        List<SysDepart> departList = departMapper.selectList(new QueryWrapper<SysDepart>().isNotNull("parent_id").eq("del_flag", "0"));
        for (SysDepart depart : departList) {
            SysDepartModel model = new SysDepartModel();
            BeanUtils.copyProperties(depart, model);
            departModelList.add(model);
        }
        // 将departModelList按departOrder按从小到大排序，为null的排在最后
        departModelList.sort(Comparator.comparing(SysDepartModel::getDepartOrder, Comparator.nullsLast(Comparator.naturalOrder())));
