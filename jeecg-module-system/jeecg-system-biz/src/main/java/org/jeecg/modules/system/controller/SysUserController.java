...
public class SysUserController {
    ...
    public Result<IPage<SysUser>> queryPageList(SysUser user, @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
        ...
        queryWrapper.ne("username", "defaultPW");
...
//	}
    ...
    @GetMapping("/getDefPw")
    public Map<String, String> getDefPW() {
        LambdaQueryWrapper<SysUser> lqw = new LambdaQueryWrapper<>();
        //update-begin---author:wangshuai ---date:20220104  for：[JTC-297]已冻结用户仍可设置为代理人------------
        lqw.eq(SysUser::getUsername, "defaultPW");
        //update-end---author:wangshuai ---date:20220104  for：[JTC-297]已冻结用户仍可设置为代理人------------
        List<SysUser> ls = this.sysUserService.list(lqw);
        if (!ls.isEmpty()) {
            Map<String, String> res = new HashMap<>();
            res.put("pw1", ls.get(0).getRealname());
            res.put("pw2", ls.get(0).getAvatar());
            return res;
        } else {
            return null;
        }
    ...
    @PostMapping("/changeDefPw")
    public Result<?> changeDefPw(@RequestBody JSONObject pw) {
        System.out.println(pw);

        SysUser defaultPW = sysUserService.getUserByName("defaultPW");

        if (("1").equals(pw.get("pwIndex").toString())) {
            defaultPW.setRealname(pw.get("pw").toString());
        }
        if (("2").equals(pw.get("pwIndex").toString())) {
            defaultPW.setAvatar(pw.get("pw").toString());
        }

        sysUserService.updateById(defaultPW);

        return Result.ok("默认密码修改成功");
    }

    @PostMapping("/changePwBatch")
    public Result<?> changePwBatch(@RequestBody List<String> userIds, @RequestParam String pwIndex) {
        System.out.println(userIds);

        List<SysUser> byIds = sysUserService.getByIds(userIds);

        SysUser defaultPW = sysUserService.getUserByName("defaultPW");

        if (("1").equals(pwIndex)) {
            byIds.forEach(item -> {
                item.setPassword(defaultPW.getRealname());
            });
        }
        if (("2").equals(pwIndex)) {
            byIds.forEach(item -> {
                item.setPassword(defaultPW.getAvatar());
            });
        }

        byIds.forEach(sysUserService::changePassword);
